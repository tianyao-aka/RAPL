...
class RetrieverDataset:
    ...
    def _get_triple_scores(
        ...
        save_dir = os.path.join('data_files', dataset_name, 'path_distance_list')
        ...
        for i in tqdm(range(len(processed_dict_list))):
            ...
            triple_scores_i, max_path_length_i = self._extract_paths_and_score(
                sample_i)
            self.path_distances.append(max_path_length_i)

        torch.save(self.path_distances, save_file)
